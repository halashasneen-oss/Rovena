...
class InspectionListFragment : Fragment(R.layout.fragment_generic_list) {

    private var _binding: FragmentGenericListBinding? = null
    private val binding get() = _binding!!

    private val viewModel: InspectionListViewModel by viewModels {
        viewModelFactory { InspectionListViewModel(appContainer, resolveVehicleId(appContainer)) }
    }

    private val adapter = InspectionAdapter { inspection ->
        findNavController().navigate(R.id.inspectionFormFragment, bundleOf("vehicleId" to inspection.vehicleId, "recordId" to inspection.id))
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.screenTitle.text = getString(R.string.hub_section_inspection)
        ...
        binding.emptyState.emptyTitle.text = getString(R.string.inspection_empty_title)
        binding.emptyState.emptyMessage.text = getString(R.string.inspection_empty_message)

        binding.fabAdd.setOnClickListener {
            val vehicleId = viewModel.uiState.value.vehicleId ?: return@setOnClickListener
            findNavController().navigate(R.id.inspectionFormFragment, bundleOf("vehicleId" to vehicleId, "recordId" to 0L))
        }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.uiState.collect { state ->
                    ...
                    val empty = !state.isLoading && state.inspections.isEmpty()
