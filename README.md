...
- **Offline/permissions/privacy/security/performance audit (P1)**: reviewed AndroidManifest permissions (no unused ones), confirmed the app makes zero network calls anywhere (offline-first is real, not just marketed), narrowed the ProGuard `-keep` rule on `presentation/**` from keeping every member name to only the class name (no reflection-based ViewModel factories exist to need more), and added debug-only `StrictMode` (`penaltyLog`, never `penaltyDeath`) to catch accidental main-thread disk/network access or leaked SQLite/Closeable objects during future development without affecting release behavior.
- **UX safety nets (P1/P2)**: audited every form/list screen for the six spec items (loading states, double-tap protection, unsaved-changes guard, delete confirmations, restore confirmations, Clear All Data) - restore already had a proper three-way choice dialog (`BackupFragment.showRestoreChoiceDialog()`) and vehicle deletion already had a confirm dialog (`GarageFragment.confirmDelete()`), but every other delete path (in-form delete buttons on Maintenance/Fuel/Expense/Document/Inspection/Reminder, and popup-menu deletes on Parts/Notes/Reminders lists) called `viewModel.delete()` directly with zero confirmation, no Save/Delete button anywhere debounced a double tap, and no form guarded against losing edits to an accidental back press or gesture. Fixed all three: a shared `Fragment.confirmDelete()` (mirroring `GarageFragment`'s existing dialog) now guards every one of those delete paths; a shared `View.setOnDebouncedClickListener()` (600ms) now guards every Save/Delete button; a shared `Fragment.guardUnsavedChanges()` registers a per-form `OnBackPressedCallback` that only intercepts system back (button or gesture) while a form-local `isDirty` flag is true, prompting "Discard changes?" instead of silently dropping edits - `isDirty` is set at the same `!isBinding` boundary each form already used to distinguish a user edit from a programmatic field set during load, so no new state machine was needed. Also added a new **Clear All Data** row under Settings → Data: a type-to-confirm dialog (the user must type the localized word for "Delete" before the button enables) that deletes every vehicle via the already-transactional, already-file-safe `VehicleRepository.deleteVehicle()` in a loop, rather than a raw `Room.clearAllTables()` that would also wipe the unrelated `app_settings` table (theme/language/PIN/etc.). The loading-state gap (no `ProgressBar` on any form) was reviewed and left alone - every form already renders near-instantly against a local Room database, so a spinner would only add visual noise with no real wait to communicate.
...
- ~~Removing a photo from the maintenance-record photo strip... does not currently delete the underlying file~~ **Fixed.** Every photo/file-owning record (maintenance photos, inspection-item photos, documents, expense receipts, the vehicle photo) now deletes its underlying file(s) on disk, not just its database row, both when a single photo/file is removed or replaced during an edit and when the whole record is deleted - mirroring the pattern `VehicleRepository.deleteVehicle()` already used for a full vehicle deletion. See `MaintenanceRepository.delete()`, `InspectionRepository.delete()`, `DocumentRepository.delete()`, `ExpenseRepository.delete()`, and the `save()`/`delete()` methods of the corresponding form ViewModels.
